...
fn bird_socket_path() -> String {
    std::env::var("BIRD_SOCKET")
        .unwrap_or_else(|_| "/tmp/ixforge-bird-test/bird.ctl".to_string())
}

#[tokio::test]
#[ignore = "requires BIRD running in Docker"]
async fn test_bird_socket_connection() {
    let client = BirdSocketClient::new(&bird_socket_path());
...
async fn test_bird_parse_real_socket_output() {
    ...
    for p in &protocols {
        assert!(p.neighbor_address.is_some(), "{} should have neighbor address", p.name);
        assert!(p.neighbor_asn.is_some(), "{} should have neighbor ASN", p.name);
    }
}
